...
TEST_F(BraveRandomDeterministicTest, Uniform_01) {
  using brave::random::deterministic::Uniform_01;
...
TEST_F(BraveRandomDeterministicTest, StdExponential) {
  using brave::random::deterministic::StdExponential;

  // Check edge cases.  Only for the smallest subnormal do we get
  // +inf or 0; these have negligible probability.
  EXPECT_DOUBLE_EQ(INFINITY, StdExponential(0, 4.9406564584124654e-324));
...
TEST_F(BraveRandomDeterministicTest, Exponential) {
  using brave::random::deterministic::Exponential;
  ...
  EXPECT_DOUBLE_EQ(INFINITY, Exponential(0, 4.9406564584124654e-324, 0.5));
  ...
  EXPECT_DOUBLE_EQ(INFINITY, Exponential(0, 4.9406564584124654e-324, 2));
...
TEST_F(BraveRandomDeterministicTest, Geometric) {
  using brave::random::deterministic::Geometric;
